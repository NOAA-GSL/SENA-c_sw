...
contains
  ...
  subroutine interpolateCalculateSpace3D(odims, interpFactor, fdims)
    ...
    !fdims(6) = ((odims(6) - odims(5) + 1) + (odims(6) - odims(5)) * interpFactor) + (odims(5) - 1)
  ...
  subroutine interpolateArray3D(oa, odims, f, fdims, interpFactor)
    ...
    ! Locals.
    integer :: i
    integer :: tmpodims(6), tmpfdims(6)

    !slices in z direction
    do i=odims(5), odims(6)
       !call interpolateArray2D(oa(:,:,i), odims, f(:,:,(i-odims(5))*(interpFactor+1)+fdims(5)), fdims, interpFactor) 
      call interpolateArray2D(oa(:,:,i), odims, f(:,:,i), fdims, interpFactor) 
    enddo

    !switch dimensions and reuse interpolateArray2D to interpolate the missing values
    !in z direction
    ! tmpodims(1) = odims(1)
    ! tmpodims(2) = odims(2)
    ! tmpodims(3) = odims(5)
    ...
    ! do i=odims(3), odims(4)
    !   call interpolateArray2D(oa(:,i,:), tmpodims, f(:,(i-odims(3))*(interpFactor+1)+fdims(3),:), tmpfdims, interpFactor) 
    ! enddo
